var Screen_EPrint_UploadMethod_File = Class.create({
	...
	initialize: function(prefix, component, evt) {
		...
		this.prefix = prefix;
		/* start: Add multiple upload if <input ...> is single upload */
		var inputid=this.prefix+'_file';
		if (! $(inputid).hasAttribute('multiple')) {
			$(inputid).setAttribute('multiple','');
		}
		/* end: Add multiple upload if <input ...> is single upload */
					...
					if (confirm(phrases['Plugin/Screen/EPrint/UploadMethod/File:confirm_bulk_upload']))
						for(var i = 0; i < files.length; ++i)
							if (files[i].size >upload_limit) {
								eprints.currentRepository().phrase ({ 'document_validate:upload_error_maxsize': {
								  'file': files[i].name, 'size': files[i].size, 'limit': upload_limit } }, 
								function(phrases) {
									alert(phrases['Plugin/Screen/EPrint/UploadMethod/File:upload_limit']);
								});
								//alert("La dimensione del file "+files[i].name+" supera il massimo di "+ upload_limit+ " bytes");
							}
							else {
			...
			for(var i = 0; i < files.length; ++i)
				if (files[i].size >upload_limit) {
					eprints.currentRepository().phrase ({ 'document_validate:upload_error_maxsize': {
					  'file': files[i].name, 'size': files[i].size, 'limit': upload_limit } }, 
					function(phrases) {
						alert(phrases['document_validate:upload_error_maxsize']);
					});
					//alert("La dimensione del file "+files[i].name+" supera il massimo di "+ upload_limit+ " bytes");
				}
				else {
	...
	finishFile: function(file) {
		...
		new Ajax.Request(url + '?ajax=1', {
			...
			onSuccess: (function(transport) {
				...
				//alert('pippo')
				new Ajax.Request(eprints_http_cgiroot + '/users/ajax/upload_validation?fileid='+file.fileid, {
				  method: 'get',
				  onException: function(req, e) {
				  	throw e;
				  },
...
function UploadMethod_file_change(input,component,prefix)
{
	input = $(input.id);
	if(input.value){
		controller.handleFiles(input.files);
		input.value='';
		return;
	}
}
